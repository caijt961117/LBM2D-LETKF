...
struct thread3d {
    ...
    void init_cuda_spec(int max_threads) {
        nx = std::min(range.x.n(), max_threads);
        auto ny_max = max_threads / nx;
        ny = std::min(range.y.n(), ny_max);
        auto nz_max = std::min(64, max_threads/ny/nx);
        nz = std::min(range.z.n(), nz_max);
